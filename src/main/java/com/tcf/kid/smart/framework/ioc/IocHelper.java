...
public class IocHelper {
	...
	//TODO TCF 启动IOC/DI依赖注入容器，基于反射给目标实例的目标属性注入属性值，基于反射调用目标实例的目标方法
	public static void init(Map<String,Map<String,Object>> fieldMap)
	{
		try
		{
			//TODO TCF 已经加载的Bean组件和Bean组件实例之间的映射Map
			...
			if(beanMap!=null)
			{
				for(Map.Entry<Class<?>,Object> beanEntry:beanMap.entrySet())
				{
					...
					if(targetClass!=null && targetInstance!=null)
					{
									...
									if(StringUtils.isNotEmpty(fieldName))
									{
										...
										if(fieldMap!=null)
										{
											for(Map.Entry<String,Map<String,Object>> fieldEntry:fieldMap.entrySet())
											{
												...
												//TODO TCF 需要注入的属性所在类simpleName必须和当前加载的类simpleName相同才能注入
						...
						//TODO TCF 以下条件可以去掉，在这里的目的是只测试AOP动态代理织入增强处理后的方法执行效果
